...
const githubMaxRetries = defaultMaxRetries
...
func (g *GithubTrending) Fetch(ctx context.Context) ([]schema.Signal, error) {
	now := g.now()
	cp := cachePath(g.cacheDir, githubTrendingName, now)
	...
	cfg := defaultRetryConfig(g.sleep)
	sigs, err := retryFetch(ctx, cfg, githubTrendingName, func() ([]schema.Signal, error) {
		return g.fetchOnce(ctx, now)
	})
	if err != nil {
		return nil, err
	}
...
func (g *GithubTrending) fetchOnce(ctx context.Context, now time.Time) ([]schema.Signal, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, &httpStatusError{Source: githubTrendingName, Status: resp.StatusCode, Body: string(body)}
	}
